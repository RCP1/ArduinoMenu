...
menu on web browser served by esp32 device
output: ESP32WebServer -> Web browser
input: ESP32WebSocket <- Web browser
format: xml, json

IMPORTANT!:
this requires the data folder to be stored on esp32 spiff
...
ESP32WebServer - https://github.com/espressif/arduino-esp32/tree/master/libraries/WebServer
...
//this version numbers MUST be the same as data/1.5
...
xmlFmt<esp32_WebServerStreamOut> serverOut(server,web_tops,webPanels);
jsonFmt<esp32_WebServerStreamOut> jsonOut(server,web_tops,webPanels);
jsonFmt<esp32BufferedOut> wsOut(web_tops,webPanels);
