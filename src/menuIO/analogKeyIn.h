...
#ifndef RSITE_ANALOGKEYIN_DEF_H
  #define RSITE_ANALOGKEYIN_DEF_H
  #include "../menuDefs.h"

  namespace Menu {

    #include "analogKeyMapDef.h"
    ...
    public:
      ...
      keyIn<N>(keyMap k[], unsigned int pin):keys(k),lastkey(-1), pin(pin) {}
      void begin() {
      }
      int available(void) {
      ...
      int peek(void) {
        ...
        for(int n=0;n<N;n++) {
          if (reading > (keys[n].value - keys[n].deviation) && reading < (keys[n].value + keys[n].deviation)) return keys[n].code;
        }
        return -1;
      }
      int read() {
